...
function testToAutoIt() {
        
    // Send data to AutoIt via postMessage
    window.chrome.webview.postMessage('testToAutoIt|Send data to AutoIt via postMessage');  
}

// _gog_Bridge.js
window.chrome.webview.postMessage('COM_TEST|hello from JavaScript');

// =======================================================================================================
// CONTEXT MENU SENSOR (Necessary for AutoIt to know which game you clicked on)
// =======================================================================================================
window.addEventListener('contextmenu', function (e) {
    ...
    if (tile) {
        ...
        if (titleEl) {
            ...
            let cleanPath = tile.href.replace(/\/$/, ""); 
            let parts = cleanPath.split('/');
            gameTitle = parts[parts.length - 1]; 
        }
    }
...
// =======================================================================================================
window.GOGBridge = {
    // Optimized version for GOG Data Extraction
    getGamesListJson: function() {
        // Εστιάζουμε μόνο στα product tiles του GOG
        const items = document.querySelectorAll('.product-tile, .product-card');
        const results = Array.from(items).map(el => {
            return {
                title: el.querySelector('.product-tile__title, .product-card__title')?.innerText.trim(),
                price: el.querySelector('.product-tile__price, .product-card__price-actual')?.innerText.trim(),
                id: el.getAttribute('data-product-id') || el.href
            };
        });
